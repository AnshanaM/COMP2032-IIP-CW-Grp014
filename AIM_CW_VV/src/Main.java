...
public class Main {
    public static void main(String[] args) {
        ...
        int probNum = 4; // can be either 0, 1, 2, 3 or 4
        ...
        Bin.binFullCapacity = problemArray[probNum].binCapacity;

        // set a centralised order of all the items for the group encoding to refer to
        Solution.itemList = problemArray[probNum].allItemsOrderedList;
        crossOverOperator.allItemsList = problemArray[probNum].allItemsOrderedList;
        ...
        for (int i = 0; i < problemArray[probNum].itemWeights.length; i++) {
            if (problemArray[probNum].itemWeights[i] > 0.5 * problemArray[probNum].binCapacity) {
                for (int k = 0; k < problemArray[probNum].noOfItems[i]; k++) {
                    itemsAboveFifty.add(problemArray[probNum].itemWeights[i]);
                }
            } else {
                for (int k = 0; k < problemArray[probNum].noOfItems[i]; k++) {
                    remainingItems.add(problemArray[probNum].itemWeights[i]);
                }
            }
        }
        ...
        System.out.println("-----------------------------------------------------------");
        problemArray[probNum].printInfo();
        System.out.println("-----------------------------------------------------------");

        // calculate lower bound for solution's number of bins
        int lowerBound = lowerBoundCalculator.lowerBound(problemArray[probNum].allItemsOrderedList);
